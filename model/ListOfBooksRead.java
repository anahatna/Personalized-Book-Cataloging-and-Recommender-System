...
public class ListOfBooksRead implements Writable {
    ...
    /// !!!
    public Genre getHighestRatedGenre() {
        HashMap<Genre, ArrayList<Double>> genreRatings = new HashMap<>();

        for (Book book : booksRead) {
            Genre g = book.getGenre();
            double rating = book.getRating(); // Assuming getRating returns double
            genreRatings.putIfAbsent(g, new ArrayList<>());
            genreRatings.get(g).add(rating);
        }

        Genre highestRatedGenre = null;
        double highestAverageRating = 0;

        for (Genre genre : genreRatings.keySet()) {
            double averageRating = genreRatings.get(genre).stream()
